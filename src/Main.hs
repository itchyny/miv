  ...
  let relname = show place
      ...
      body = "" : (if isAllAscii then [] else [ "scriptencoding utf-8", "" ])
         ...
         <> [ ""
            ...
            , "unlet s:save_cpo" ]
  time <- getZonedTime
  oldHash <- fileHash path
  when (oldHash /= Just (sha1Hash body)) $
...
sha1Hash :: [Text] -> Text
sha1Hash = show . (Crypto.hash :: Char8.ByteString -> Crypto.Digest Crypto.SHA1) . TE.encodeUtf8 . T.concat

fileHash :: String -> IO (Maybe Text)
fileHash path = do
  ...
  case eitherFile of
       Right file -> do
         ...
         hClose file
         return $ Just $ sha1Hash $ dropWhile ("\" " `T.isPrefixOf`) contentLines
